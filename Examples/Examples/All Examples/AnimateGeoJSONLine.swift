...
public class AnimateGeoJSONLine: UIViewController, ExampleProtocol {
    ...
    func addLine() {
        ...
        routeLineSource.data = .feature(Feature(LineString(allCoordinates)))
        ...
        lineLayer.paint?.lineWidth = .constant(6.0)
    ...
    func animatePolyline() {
        ...
        Timer.scheduledTimer(withTimeInterval: 0.10, repeats: true) { ( timer ) in
            ...
            if currentIndex > self.allCoordinates.count {
                timer.invalidate()
                return
            }
            ...
            currentCoordinates = Array(self.allCoordinates[0..<currentIndex - 1])
